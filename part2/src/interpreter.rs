...
impl Interpreter {
    ...
    pub fn expr(&mut self) -> Result<i64, String> {
        ...
        if let Token::OPERATION(operation) = operation {
            match operation {
                Operation::PLUS => return Ok(left.into_int()? + right.into_int()?),
                Operation::MINUS => return Ok(left.into_int()? - right.into_int()?),
            }
        }
    ...
    fn next_token(&mut self) -> Result<Token, String> {
        ...
        match current_character.as_ref() {
            "+" => {
                return Ok(Token::OPERATION(Operation::PLUS));
            },
            "-" => {
                return Ok(Token::OPERATION(Operation::MINUS));
            },
